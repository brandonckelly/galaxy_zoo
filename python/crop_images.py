...
print 'Doing file...'
for file in training_files[0:10]:
    print file, '...'
    ...
    for c in range(3):
        ...
        peak_threshold = np.median(this_im) + 5.0 * flux_sigma  # only look for 5-sigma peaks
        coords = peak_local_max(this_im, min_distance=50, threshold_abs=peak_threshold, exclude_border=False)
        ...
        for i in xrange(nsources):
            ...
            if i == 0:
                ...
                # contaminants, make make initial guess of sigmas no larger than 20 pixels
                Mxx_min = 20.0 ** 2
                Myy_min = 20.0 ** 2
            Mxx = min(Mxx, Mxx_min)
            Myy = min(Myy, Myy_min)

            amp = this_im[centroid[0], centroid[1]]
        ...
        xgrid, ygrid = np.mgrid[:ndim[0], :ndim[1]]

        params, success = optimize.leastsq(sum_of_gaussians_error, iparams,
                                           args=(image_fit.ravel(), xgrid.ravel(), ygrid.ravel(),
                                                 xcentroids, ycentroids), ftol=1e-2, xtol=1e-2)
        ...
        for k in range(nsources):
            ...
            amajor[k], aminor[k] = 2 * np.sqrt(vals)  # major and minor axes of ellipse
        ...
        xrange = 4.0 * np.abs(gauss_params['xsigma'][0])
        yrange = 4.0 * np.abs(gauss_params['ysigma'][0])
        xmin = int(coords[0, 0] - xrange)
        if xmin < 0:
            xmin = 0
        xmax = int(coords[0, 0] + xrange)
        if xmax > ndim[0]:
            xmax = ndim[0]
        ymin = int(coords[0, 1] - yrange)
        if ymin < 0:
            ymin = 0
        ymax = int(coords[0, 1] + yrange)
        if ymax > ndim[1]:
            ymax = ndim[1]
        ...
        for k in range(nsources):
            ...
            x_ell, y_ell = ellipse(amajor[k], aminor[k], rotang[k], xcentroids[k], ycentroids[k])
        ...
        # TODO: still need to median filter anything outside of 4-sigma
    ...
    dataframe.to_csv(data_dir + source_id + '_gauss_params.csv')

    end_time = datetime.datetime.now()

    tdiff = end_time - start_time
    tdiff = tdiff.total_seconds()
    print 'Doing', tdiff, 'seconds / galaxy.'
    print training_files * tdiff / 60.0 / 60.0, 'hours left.'